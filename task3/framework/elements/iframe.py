...
class Iframe:
    ...
    def switch_into(self):
        ...
        LoggerUtils.log_debug(f"{self.name} - switching into the frame")
        Swd.get_driver().switch_to.frame(iframe)
        return iframe

    def switch_out_of(self):
        LoggerUtils.log_debug(f"{self.name} - switching out of the frame")
        Swd.get_driver().switch_to.default_content()

    def get_text(self):
        LoggerUtils.log_info(f"{self.name} - getting text from the frame")
        self.switch_into()
    ...
    def get_text_from_nested_iframe(self):
        LoggerUtils.log_debug(f"{self.name} - getting text from nested frame")
        self.switch_into()
